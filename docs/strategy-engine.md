...
Lift-and-coast can be configured as `inferred` or `fixed`. Inferred mode searches for the fuel-saving percentage needed to extend the stint or remove a stop. Fixed mode applies the selected percentage to every lift-and-coast candidate; the default mode is fixed with a selected target of 3%.
