...
const initialConfiguration = (): PerformanceReportConfiguration => ({ language: "en", detail_level: "detailed", include_charts: true, anonymize_driver: false, title: "", driver_name: "", team_name: "", notes: "" });
...
export function PerformanceReportDialog({ session, onClose }: Props) {
  ...
  const generate = async (next = configuration) => {
    ...
    try {
      await run<PerformanceReportRecord>(() => api.startPerformanceReportJob(session.id, { ...next, detail_level: "detailed", include_charts: true }));
      await loadReports();
    } catch (exc) {
      ...
      <section className="report-modal" role="dialog" aria-modal="true" aria-labelledby="performance-report-title">
        ...
        <div className="report-modal-grid">
          <div className="report-options">
            ...
            <div className="toggle-row"><span>All analysis sections and every chart supported by the recorded data are included automatically.</span><label><input type="checkbox" checked={configuration.anonymize_driver} onChange={(event) => update("anonymize_driver", event.target.checked)} /> Anonymize driver name</label></div>
          </div>
