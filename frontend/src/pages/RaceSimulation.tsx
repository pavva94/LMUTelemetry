...
function MonteCarloStrategyCard({ summary, selected, onSelect }: { summary: RaceSimulationSummary; selected: boolean; onSelect: () => void }) {
  return <section className={`card span-4 strategy-card${selected ? " selected" : ""}`}>
    ...
    <h2>{summary.name}</h2>
    <div className="strategy-card-main"><strong>{formatRaceTime(summary.median_time)}</strong><span>Median race time · P90 {formatRaceTime(summary.p90)}</span></div>
    <div className="header-grid two">
      <div><span className="label">Mean</span><strong>{formatRaceTime(summary.mean_time)}</strong></div>
...
export function MonteCarloStrategyPanel({ sessionId, assumptions }: { sessionId: string; assumptions: MonteCarloAssumptions }) {
  ...
  return <>
    ...
    {!result ? <section className="card span-12"><div className="empty-state"><strong>Ready to simulate</strong><span>Run the model to compare strategy outcomes and risk using the current Planner inputs.</span></div></section> : <>
      ...
      <section className="card span-12"><SectionTitle title="Strategy comparison" help="Mean, median, downside, and reliability are calculated from the Monte Carlo samples." /><div className="table-wrap"><table><thead><tr><th>Strategy</th><th>Stops</th><th>Mean</th><th>Median</th><th>P90</th><th>Fastest</th><th>Fuel risk</th><th>Tyre risk</th></tr></thead><tbody>{result.summaries.map((summary) => <tr key={summary.name}><td>{summary.name}</td><td>{summary.stops}</td><td>{formatRaceTime(summary.mean_time)}</td><td>{formatRaceTime(summary.median_time)}</td><td>{formatRaceTime(summary.p90)}</td><td>{fmt(summary.fastest_probability * 100, 0)}%</td><td>{fmt(summary.fuel_risk_probability * 100, 0)}%</td><td>{fmt(summary.tyre_risk_probability * 100, 0)}%</td></tr>)}</tbody></table></div></section>
      <section className="card span-6"><SectionTitle title="Outcome distribution" help="P5, median, and P90 expose both expected time and downside." /><ResponsiveContainer width="100%" height={260}><BarChart data={result.summaries.map((summary) => ({ name: summary.name, p5: summary.p5, median: summary.median_time, p90: summary.p90 }))}><CartesianGrid stroke="#27313a" /><XAxis dataKey="name" /><YAxis tickFormatter={(value) => formatRaceTime(value)} /><Tooltip formatter={(value) => formatRaceTime(Number(value))} /><Legend /><Bar dataKey="p5" fill="#6dd6ff" /><Bar dataKey="median" fill="#e6b450" /><Bar dataKey="p90" fill="#ff8c69" /></BarChart></ResponsiveContainer></section>
...
export function RaceSimulation() {
  ...
  return <div className="page grid">
    ...
    {!result ? <section className="card span-12"><div className="empty-state"><strong>{t("raceSimulation.awaitingRun")}</strong><span>{t("raceSimulation.awaitingRunDetail")}</span></div></section> : <>
      ...
      <section className="card span-12"><SectionTitle title={t("raceSimulation.comparison")} help={t("raceSimulation.comparisonHelp")} /><div className="table-wrap"><table><thead><tr><th>{t("raceSimulation.strategy")}</th><th>{t("raceSimulation.stops")}</th><th>{t("raceSimulation.mean")}</th><th>{t("raceSimulation.median")}</th><th>{t("raceSimulation.p90")}</th><th>{t("raceSimulation.fastest")}</th><th>{t("raceSimulation.fuelRisk")}</th><th>{t("raceSimulation.tyreRisk")}</th></tr></thead><tbody>{result.summaries.map((summary) => <tr key={summary.name}><td>{summary.name}</td><td>{summary.stops}</td><td>{formatRaceTime(summary.mean_time)}</td><td>{formatRaceTime(summary.median_time)}</td><td>{formatRaceTime(summary.p90)}</td><td>{fmt(summary.fastest_probability * 100, 0)}%</td><td>{fmt(summary.fuel_risk_probability * 100, 0)}%</td><td>{fmt(summary.tyre_risk_probability * 100, 0)}%</td></tr>)}</tbody></table></div></section>
      <section className="card span-6"><SectionTitle title={t("raceSimulation.distribution")} help={t("raceSimulation.distributionHelp")} /><ResponsiveContainer width="100%" height={260}><BarChart data={result.summaries.map((summary) => ({ name: summary.name, p5: summary.p5, median: summary.median_time, p90: summary.p90 }))}><CartesianGrid stroke="#27313a" /><XAxis dataKey="name" /><YAxis tickFormatter={(value) => formatRaceTime(value)} /><Tooltip formatter={(value) => formatRaceTime(Number(value))} /><Legend /><Bar dataKey="p5" fill="#6dd6ff" /><Bar dataKey="median" fill="#e6b450" /><Bar dataKey="p90" fill="#ff8c69" /></BarChart></ResponsiveContainer></section>
